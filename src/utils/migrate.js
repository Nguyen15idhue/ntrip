...
import { User, Location, Station, Rover } from '../models/index.js';
...
async function migrate() {
  try {
    ...
    const adminUser = await User.create({
      ...
      role: 'admin',
      password_hash: 'admin123' // This will be hashed by the model hook
    });
    logger.info(`Created admin user: ${adminUser.email}`);

    // Create Vietnam provinces
    const locations = await Location.bulkCreate(vietnamProvinces);
    logger.info(`Created ${locations.length} provinces in Vietnam.`);

    // Create a sample station
    const hanoi = await Location.findOne({ where: { province_name: 'Hà Nội' } });
    const sampleStation = await Station.create({
      name: 'HANOI_VRS',
    ...
    logger.info(`Created sample station: ${sampleStation.name}`);

    // Create a sample rover account
    const sampleRover = await Rover.create({
      username: 'rover1',
      password_hash: 'rover123', // This will be hashed by the model hook
      station_id: sampleStation.id,
      user_id: adminUser.id,
      status: 'active',
      description: 'Demo Rover'
    });
    logger.info(`Created sample rover: ${sampleRover.username}`);
